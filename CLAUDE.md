...
All config is centralized in `config.py`. Paths (`DATA_DIR`, `DB_PATH`, `CHROMA_PATH`) and `OPENROUTER_BASE_URL` are plain module constants; everything a user can change is resolved lazily through a module-level `__getattr__` (PEP 562) in priority order: `data/settings.json` (written by the in-app Setup page) → env/`.env` → built-in defaults. That means `config.ESPN_LEAGUE_ID` re-reads on every access, so saving from the Setup page (or switching the active league) takes effect without restarting the server — **don't convert these back into module-level constants**, and don't add new user-settable values without adding them to `LEAGUE_FIELDS` (if it varies per league) or `GLOBAL_SETTING_KEYS` (if it's shared) — both are folded into `SETTING_KEYS`. `save_settings()` merges (ignoring `None`) and writes atomically, so the UI can omit unchanged secrets rather than blanking them.
...
**`tools/*.py`** — each exposes the same two-symbol interface: `TOOL_SCHEMA` (a tool definition dict: `name`/`description`/`input_schema`) and `run_tool(tool_input: dict)`. `query_stats.py` validates `sort_by` against an allow-list before interpolating it into SQL (it's a column identifier, can't be parameterized) — extend that allow-list rather than removing the check if adding sortable columns. `query_roster.py` scopes every `teams`/`rosters`/`matchups` query by `league_id = str(config.ESPN_LEAGUE_ID)` (the active league) — including the `team_names` lookup used to label matchups, since `team_id` values are only unique within a league, not across leagues sharing the same database. `query_stats.py`/`search_news.py` stay unscoped (nflverse/RSS data isn't league-specific).
