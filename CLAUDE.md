...
**`tools/*.py`** — each exposes the same two-symbol interface: `TOOL_SCHEMA` (Anthropic-style tool definition: `name`/`description`/`input_schema`) and `run_tool(tool_input: dict)`. `query_stats.py` validates `sort_by` against an allow-list before interpolating it into SQL (it's a column identifier, can't be parameterized) — extend that allow-list rather than removing the check if adding sortable columns.

**`agent/chat.py`** — a manual (non-SDK-runner) tool-calling loop. Runs against **OpenRouter** (OpenAI-compatible Chat Completions API), currently defaulting to `qwen/qwen3.7-flash` via `config.OPENROUTER_MODEL` — not Anthropic directly, despite the tools' Anthropic-shaped schemas and the `anthropic` package still being a dependency. `_to_openai_tool()` adapts each `TOOL_SCHEMA` into OpenAI's `{"type": "function", "function": {...}}` shape at call time. `ANTHROPIC_API_KEY`/`ANTHROPIC_MODEL` are still wired in `config.py` (unused by `chat.py` today) if the agent is ever pointed back at Claude directly — that would mean reverting to the Anthropic Messages API shape (streaming, `tool_use`/`tool_result` blocks) rather than the OpenAI shape currently in `run_turn()`.
